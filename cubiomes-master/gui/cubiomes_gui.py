...
class CubiomesGUI(tk.Tk):
    def __init__(self):
        ...
        self.seed_tab = self._build_seed_tab(notebook)
        ...
        notebook.add(self.seed_tab, text="シード逆算")
    ...
    def _build_seed_tab(self, parent):
        ...
        ttk.Label(form, text="構造物:").grid(row=0, column=0, sticky="w")
        self.seed_struct_var = tk.StringVar(value=api.STRUCTURES[0][0])
        ttk.Combobox(
            form, textvariable=self.seed_struct_var, state="readonly", width=26,
        ...
        ttk.Button(form, text="シードを探索",
                   command=self._on_find_seed).grid(
            row=3, column=3, padx=10)

        note = (
            "指定した構造物タイプの候補位置から、指定範囲内のワールドシードを探索します。\n"
            "探索範囲を広げると時間がかかるため、最初は小さな範囲で試してください。"
        )
        ...
        self.seed_tree.heading("no", text="#")
        self.seed_tree.heading("seed", text="Seed")
        self.seed_tree.column("no", width=50, anchor="center")
    ...
    def _parse_positions(self, text):
        positions = []
        for line in text.splitlines():
            ...
            if len(parts) != 2:
                raise ValueError(
                    f"座標は 'X,Z' または 'X Z' 形式で1行ごとに指定してください: {line}")
            x = int(parts[0])
            z = int(parts[1])
            positions.append((x, z))
        if not positions:
            raise ValueError("座標リストを1つ以上指定してください。")
        return positions

    def _on_find_seed(self):
        struct_name = self.seed_struct_var.get()
        struct_type = None
        ...
        if end_seed < start_seed:
            messagebox.showerror("入力エラー", "終了シードは開始シード以上で指定してください。")
            return
        if tolerance < 0 or tolerance > 1024:
            messagebox.showerror("入力エラー", "許容誤差は0〜1024の範囲で指定してください。")
            return
        ...
        def worker():
            try:
                found, seed = api.find_seed_for_structure(
